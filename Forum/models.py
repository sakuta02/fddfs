...
class CustomArticles(models.Model):
    ...
    created_by = models.CharField(max_length=255, verbose_name='Автор поста')
    genre = models.ForeignKey('Genre', on_delete=models.PROTECT, null=True, verbose_name='Жанр')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='Дата создания')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Дата обновления')
    is_published = models.BooleanField(choices=tuple(map(lambda x: (bool(x[0]), x[1]), Status.choices)), default=Status.PUBLISHED, verbose_name='Опубликованно')
    text = models.TextField(default='Эта статья была опубликованна без текста', verbose_name='Текст статьи')
    published = PManager()
    objects = models.Manager()
